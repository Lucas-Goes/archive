...
export function RegisterForm({ onSuccess }: Props) {
  ...
  const [isSuccess, setIsSuccess] = useState(false); // Feedback de sucesso
  ...
  async function handleSubmit(formData: FormData) {
    ...
    if (res && "success" in res && res.success) {
      // 1. Ativa feedback visual
      setIsSuccess(true);
      
      // 2. Aguarda um pouco para o usuário ler a mensagem
      setTimeout(() => {
    ...
    <form action={handleSubmit} className="space-y-4">
      ...
      {isSuccess && (
        <div className="p-4 rounded-lg bg-green-500/10 border border-green-500/50 text-green-400 text-sm text-center animate-in fade-in zoom-in duration-300">
          ...
          <p className="text-xs opacity-80">Redirecionando para confirmação de e-mail...</p>
        </div>
      )}

      {/* Inputs só aparecem se não houver sucesso ainda */}
      {!isSuccess && (
        <>
          <input
            name="name"
          ...
          <div className="relative">
            ...
            <div className="absolute right-3 top-1/2 -translate-y-1/2 text-xs pointer-events-none">
              {usernameStatus === "checking" && <span className="text-white/30">•••</span>}
              {usernameStatus === "available" && (
                <svg className="text-green-500" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4">
                  <path d="M5 13l4 4L19 7" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              )}
              {usernameStatus === "taken" && (
                <svg className="text-red-500/60" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4">
...
const EyeIcon = () => (
  <svg xmlns="http://www.w3.org" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="w-4 h-4" strokeWidth="2.5">
